...
from app.schemas.PaymentRequest import PaymentRequest
from app.repository.nursery_repository import (
    get_plant_by_id,
    decrease_stock,
    get_nursery_by_user_id,
)
...
async def add_order(db: Session, order: OrderCreate):
    ...
    new_order = OrderModel(
        ...
        total_amount=order.quantity * int(plant.price),
...
async def get_all_order(db: Session, nursery_id: int, skip: int = 0, limit: int = 20):
    ...
    orders = (
        db.query(OrderModel, PlantModel, UserModel)
        .join(PlantModel, OrderModel.plant_id == PlantModel.plant_id)
        .join(UserModel, OrderModel.user_id == UserModel.user_id)
        .filter(PlantModel.nursery_id == nursery.nursery_id)
        .filter(OrderModel.status != "Delivered")
...
async def get_all_order_by_user_id(
    db: Session, user_id: int, skip: int = 0, limit: int = 20
):
    orders = (
        db.query(OrderModel, PlantModel)
        .join(PlantModel, OrderModel.plant_id == PlantModel.plant_id)
...
async def get_order_by_id(db: Session, order_id: int):
    ...
    user = {
        "name": order_detail[2].name,
        "address": order_detail[2].address,
        "contact_number": order_detail[2].contact_number,
    }
    return {
        "order_id": order_detail[0].order_id,
        "Plant name": order_detail[1].name,
        "qunatity": order_detail[0].quantity,
        "Total Amount": order_detail[0].total_amount,
        "Status": order_detail[0].status,
        "Created_at": order_detail[0].created_at,
        "customer": user,
    }
...
async def change_order_status(order_id: int, status: str, db: Session):
    ...
    if not OrderStatusService.validate_status_change(order.status, status):
        raise HTTPException(
            status_code=400,
            detail=f"Invalid status transition from {order.status} to {status}",
        )
    order.status = status
    try:
        db.commit()
