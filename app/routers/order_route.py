...
async def get_orders(
    nursery_id: int, skip: int = 0, limit: int = 20, db: Session = Depends(get_db)
):
...
async def payment_intent(payment: PaymentRequest):
    return await create_payment_intent(payment)
